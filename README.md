...
### 结果展示
本实验在 ESC-50 数据集上进行，对比了不同相似度度量方法在不同帧长([40, 25, 15]ms)和对应的不同帧移([20, 10, 5]ms)下的准确率表现情况。

**Top10-Hit-Accuracy**
| Metric | DTW | Cosine  | L2 |
| :--- | :---: | :---: | :---: |
| **(40, 20) ms** | 63.50% | 63.50% | 68.75% |
| **(25, 10) ms** | 63.25% | 65.00% | **68.00** |
| **(15, 05) ms** | 63.75% | 64.50% | 67.75% |

**Top20-Hit-Accuracy**
| Metric | DTW | Cosine  | L2 |
| :--- | :---: | :---: | :---: |
| **(40, 20) ms** | 78.75% | 77.50% | 80.00% |
| **(25, 10) ms** | 77.00% | 78.00% | 79.50% |
| **(15, 05) ms** | 79.25% | 78.00% | **80.25%** |

**Score-Accuracy**
| Metric | DTW | Cosine  | L2 |
| :--- | :---: | :---: | :---: |
| **(40, 20) ms** | 25.00% | 31.00% | 31.75% |
| **(25, 10) ms** | 23.75% | **32.00%** | 31.50% |
| **(15, 05) ms** | 23.00% | 30.25% | 28.75% |
...
在实验结果中可以看到，在 Top-N Hit 任务中，**L2 距离的表现普遍优于 Cosine 和 DTW**；而在 Score 任务中，**Cosine 相似度在特定参数下表现最佳**，L2 紧随其后。

虽然理论上 DTW 能够处理音频信号在时间轴上的非线性伸缩，但在本实验的 ESC-50 数据集上，简单的 L2 和 Cosine 度量反而取得了更好的效果。这可能是因为 ESC-50 数据集的音频片段长度固定，且同类样本在整体频谱特征上具有较高的一致性。L2 和 Cosine 距离计算效率更高，且在特征对齐较好的情况下能有效衡量相似度。

#### 不同帧长与帧移对比试验

不同帧长与帧移的选择对 MFCC 特征提取及分类准确率有显著影响。

**帧长决定了频率分辨率**。较长的帧长能提供更好的频率分辨率，但会模糊短促的时域变化；较短的帧长则能捕捉快速的时变特征。
**帧移决定了时间分辨率**。较小的帧移能提供更平滑的特征序列，但增加了计算量和特征维度。

从表格数据可以看出：
*   **Top-N Hit 任务**：在 Top20 Hit 中，(15, 5) ms 的参数组合配合 L2 距离达到了最高的 80.25% 准确率。这表明对于宽泛的检索任务，较高的细粒度（较短的帧长和帧移）有助于捕捉更多的特征细节，从而提高命中率。
*   **Score 任务**：在对精度要求更高的 Score 分类任务中，最优结果出现在 (25, 10) ms 配合 Cosine 相似度 (32.00%)。这说明适中的帧长和帧移能够在保留足够特征信息的同时，避免过多的冗余和噪声干扰，从而在严格的分类打分中表现更好。

总体而言，(25, 10) ms 是一个较为稳健的参数选择，在各项指标上都取得了不错的效果。
...
随后，对获得的mfcc特征进行倒谱提升lifter。具体而言，其根据mfcc特征长度设置了一组类似正弦变化的权重参数，用于重点突出中间阶次的MFCC系数，让相似度函数聚焦于每段音频最为重点的部分。具体公式如下：

\[
w_n = 1 + \frac{L}{2}sin(\frac{\pi n}{L})
\]

由于声音是一个连续变化的过程，不仅仅是静态的频谱。例如，一个音从低变高，或者一个元音过渡到辅音，这些变化的过程往往比静态的频谱更能区分不同的声音。对于倒谱提升加权后的mfcc特征，我们还希望可以观察到音频随时间变化过程的突变。因此我们又应用mfcc特征的一二阶差分作为特征，与原始的mfcc特征拼接后用于计算相似度。

最终，将拼接后的MFCC特征矩阵归一化后保存为pt文件，便于后续更换不同相似函数评测时加速。
